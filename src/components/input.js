...
export default function Input({label, type = 'text', ...props}){
    ...
    return (
        <label className="block relative bg-zinc-50 border rounded-sm focus-within:border-gray-400">
            ...
            {type === 'password' && field.value && (
                <div onClick={() => setShow(show => !show)} className="absolute top-0 right-0 h-full cursor-pointer select-none flex items-center text-sm font-semibold pr-2">
                    {show ? 'Hide' : 'Show'}
                </div>
            )}
            {meta.error}
